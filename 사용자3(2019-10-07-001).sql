...
-- 테이블을 처음 생성할당시에 미처 생각지 못한 칼럼이 필요한 경우가 있다.
...
--      그렇지 않은 경우도 있는데 이때는 저장된 데이터의 일부가 잘릴 수 있다.
-- 나. 타입변경
...
-- 현재 book 테이블에 추가된 데이터들에 장르칼럼이 비어 있는 상태이다
-- 장르칼럼의 데이터를 채워 넣기
SELECT * FROM tbl_books;

-- UPDATE를 실행할때 2개의 이상의 레코드에 영향을 미치도록
-- 명령을 수행하는 것은 특별한 경우가 아니면 지양을 하자
-- UPDATE 수행은 먼저 PK를 확인 한 후 WHERE 절에 PK조건을 설정하여 하자
UPDATE tbl_books
SET b_genre = '데이터베이스'
...
SELECT * 
FROM [table1], [table2]
WHERE tbale1.col = table2.col;
-- 완전Join, EQ Join 이라고 하며
-- 결과를 카티션곱 이라고 표현한다.
-- table1과 table2를 Relation 할때
